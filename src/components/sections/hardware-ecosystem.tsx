...
function DeviceVisual({ device, isHovered }: { device: typeof devices[0]; isHovered: boolean }) {
  return (
    <div className="relative w-full h-64 mb-10 flex items-center justify-center">
      {/* Subdued Halo Effect */}
      <motion.div
        className="absolute w-48 h-48 rounded-full blur-[80px] z-0"
        style={{
      ...
      {/* Static Device Image — with soft drop-shadow */}
      <div className="relative z-10 w-full h-full flex items-center justify-center">
        <div className="relative w-44 h-44 sm:w-48 sm:h-48">
          <Image
            ...
            alt={device.name}
            fill
            className="object-contain transition-transform duration-700 group-hover:scale-110 drop-shadow-[0_8px_16px_rgba(0,0,0,0.08)]"
            priority
          />
        </div>
      </div>
